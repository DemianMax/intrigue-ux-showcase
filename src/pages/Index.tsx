    ...
    <div className="flex flex-col min-h-screen bg-background relative font-inter">
      ...
      <nav className="fixed top-0 left-0 w-full bg-background/80 backdrop-blur z-30 border-b border-border shadow-sm">
        <ul className="flex items-center justify-center gap-7 py-3 text-brand-dark font-semibold text-base">
          <li className="cursor-pointer hover:text-brand-accent transition" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>Início</li>
      ...
      <section className="min-h-[100dvh] flex flex-col justify-center items-center gap-8 relative pt-24 sm:pt-28">
        ...
        >
          ...
          <motion.span className="block h-1 w-16 mx-auto mt-2 rounded-full bg-brand-accent"
            initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ delay: 0.6, duration: 0.6, type: "spring" }} style={{ transformOrigin: "left" }}/>
        </motion.h1>
        <AnimatePresence>
          <motion.p
        ...
        <motion.button
          ...
          className="absolute left-1/2 -translate-x-1/2 bottom-10 animate-bounce bg-brand-accent/10 hover:bg-brand-accent/20 rounded-full p-2 transition outline-none ring-0 ring-brand-accent/50"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project) => (
            ...
            >
              ...
              <button
                className="mt-auto px-6 py-2 rounded-full bg-brand-accent text-white font-semibold shadow hover:bg-brand-dark/90 transition"
