...
const Index = () => {
  ...
  return <div className="relative font-inter bg-background text-foreground">
      ...
      <div className="relative">
        {sections.map((section, index) => <div key={index} className="bg-slate-300">
            <div id={`section-${index}`} className={`w-full flex items-center justify-center relative ${index === 0 ? "min-h-screen" : ""} ${section.bgClass}`}>
              {section.component}
            </div>
          </div>)}
      </div>
