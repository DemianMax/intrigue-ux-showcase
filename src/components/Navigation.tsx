...
const NAV_ITEMS = [
  { labelKey: "navHome", fallback: "Home", href: "/" },
  { labelKey: "navAbout", fallback: "Sobre", href: "/" },
  { labelKey: "navProjects", fallback: "Projetos", href: "/" },
  { labelKey: null, fallback: "Portfólio", href: "/" },
  { labelKey: null, fallback: "Skills", href: "/" },
  { labelKey: null, fallback: "Currículo", href: "/curriculo" },
  { labelKey: "navContact", fallback: "Contato", href: "/" }
];
...
const Navigation: React.FC<NavigationProps> = ({ onSectionScroll }) => {
  ...
  const handleNavClick = (item: typeof NAV_ITEMS[0], index: number) => {
    if (item.href === "/curriculo") {
      // Link direto para currículo
      navigate("/curriculo");
      setMobileMenuOpen(false);
    } else if (item.href === "/" && onSectionScroll) {
      // Scroll para seção na home
      onSectionScroll(index);
      setMobileMenuOpen(false);
    } else {
      // Navegar para home
      navigate("/");
